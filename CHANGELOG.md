...
- A restore now treats a pinned object that is gone (HTTP 404 or 410) or a refused range request as drift, so a configured mirror restores it instead of the pull failing; a mirror that cannot be reached is reported with the drift rather than aborting the run. In Python, `pull()` now raises `UpstreamChanged` for these instead of `httpx.HTTPStatusError` or `RangeNotHonored`; the CLI still exits 4 and now lists them as drift (`gone upstream (404)`, `range refused`). ([#339](https://github.com/jakeryderv/usdata/issues/339))
- Every datetime in the public models (`TimeRange`, `Provenance`, `TemporalSelection`, `Lockfile`, `CacheEntry`, and inspection results) is now stored as aware UTC, reading a naive value as UTC as the rest of usdata does. Models built directly in Python no longer raise `TypeError`: a `TimeRange` mixing naive and aware bounds, a registry search with a naive window, a sidecar with a naive `retrieved_at` during `usdata cache prune --older-than`, and adapters listing a mix of naive and aware start times now all behave as UTC. Code that read a naive datetime back from a model, or compared one with a naive value, now gets an aware UTC one. ([#351](https://github.com/jakeryderv/usdata/issues/351))
- `usdata fetch --dry-run` now exits 1 when no asset matched, as `fetch`, `search`, and `datasets` already do, so a script can use a dry run to test whether data exists; a script that treated a dry run's exit 0 as success must now allow 1. It still prints `0 asset(s) matched`, and `--json` still prints `[]`. ([#355](https://github.com/jakeryderv/usdata/issues/355))
- Search text with no letters or digits, such as `usdata search '?'`, is now refused as bad input (exit 2, or `ValueError` from `usdata.search`) instead of listing every dataset as a match. Blank or absent text still applies no keyword filter. ([#357](https://github.com/jakeryderv/usdata/issues/357))

### Fixed

- A forced re-resolve or an `update` fetches an asset that two manifest sources share once, so both lockfile entries always pin the bytes the cache holds, even if upstream republishes during the run. ([#340](https://github.com/jakeryderv/usdata/issues/340))
- `location="Alaska"` and `location="Aleutians West Census Area, AK"` now resolve to their western-hemisphere part, not a box spanning 359 degrees of longitude that sent adapters a band around the whole globe and returned earthquakes in Kamchatka and Iceland. The Near Islands and part of the Rat Islands, which lie in the eastern hemisphere, are left out; pass a `bbox` for them. The place generator refuses to write any box that crosses the antimeridian, and the bundled `places.sources.json` records each clipped place and the box it dropped. ([#341](https://github.com/jakeryderv/usdata/issues/341))
- `epa:aqs-daily` now paces transport retries as well as first requests: a retry after a 429, a 5xx, or a timeout waits until six seconds have passed since the previous attempt, or longer when `Retry-After` asks, instead of reaching EPA again within about a second. `usdata.protocols.http.get` gains a `before_attempt` callable for this, run before every attempt. ([#342](https://github.com/jakeryderv/usdata/issues/342))
- A date alone as the end of a window now covers that whole UTC day in every ISO 8601 spelling Python reads, not only `2024-01-15`: the compact `20240115` and the week form `2024-W03-1` used to resolve to midnight and silently drop the last day. ([#343](https://github.com/jakeryderv/usdata/issues/343))
- Upstream failures from `epa:aqs-daily`, `fema:disaster-declarations`, and `usgs:earthquakes` now exit 4, not 2. An AQS HTTP error whose body gives no header reason, such as a 503 maintenance page, stays an HTTP error instead of becoming `AqsError`, and a response that is not the JSON or count the service promises raises `httpx.DecodingError` instead of `QueryError` or `JSONDecodeError`. An AQS refusal with a header reason, such as a bad key, is still an `AqsError` and exits 2. ([#344](https://github.com/jakeryderv/usdata/issues/344))
- NetCDF4 and GRIB2 readers no longer stamp registry units on a decoded time variable, whose units the file stated and xarray moved into its encoding, so a GLM Dataset opened with `open_netcdf` writes back with `to_netcdf()`. ([#345](https://github.com/jakeryderv/usdata/issues/345))
- NBM probability-threshold messages can now be selected: a `messages` selector's further text is everything after the step text, colons included, so `APCP:surface:0-1 hour acc fcst:prob >0.254:prob fcst 255/255` selects that threshold and every index line reads back as a selector for itself. A selector whose short name and level match but whose step or further text does not now lists the steps or further texts published there. ([#346](https://github.com/jakeryderv/usdata/issues/346))
- The `noaa:nexrad-level2` and `noaa:nexrad-level3` catalog extents now reach east to 27°W, so searching at the Lajes radar in the Azores (`LPLA`) finds them. The Guam, Korea, and Okinawa radars (`PGUA`, `RKJK`, `RKSG`, `RODN`) lie beyond the antimeridian, which a single extent cannot wrap across; the NEXRAD guide and catalog descriptions now say so, and those radars remain available by site id. ([#347](https://github.com/jakeryderv/usdata/issues/347))
- Updating a pinned GRIB2 partial asset whose object was republished now fails with a message saying to re-resolve with `--force`, instead of a bare "republished" error. ([#348](https://github.com/jakeryderv/usdata/issues/348))
- The committed example manifests and lockfiles are checked out with LF line endings on every platform, so a Windows checkout no longer reports each example's manifest as changed. ([#350](https://github.com/jakeryderv/usdata/issues/350))
- `usdata cache` and `cache prune` no longer list or delete the temporary files an interrupted or concurrent download leaves beside the cache, nor anything in the staging directory. ([#352](https://github.com/jakeryderv/usdata/issues/352))
- GRIB2 `units` holding a non-integer power, such as ecCodes' `m**(2/3) s**-1` for eddy dissipation, now keep the file's spelling instead of becoming the invalid `m(2/3) s-1`; only integer powers of a unit symbol take the UDUNITS notation. ([#353](https://github.com/jakeryderv/usdata/issues/353))
- `Grib2Summary.variable_for` now names a message ecCodes has no short name for as the GRIB2 reader does, `parameter_<discipline>_<category>_<number>` or the MRMS product, so the name it returns is a variable in the opened Dataset; each inspected `GribMessage` carries that name as `base_name`. For a selector that fetched a message holding several fields, such as RAP's wind components, it now raises and lists their variables instead of returning the first one. ([#354](https://github.com/jakeryderv/usdata/issues/354))
- `epa:aqs-daily` and `usgs:earthquakes` now send bounding-box edges to six decimal places, about 0.1 m, instead of six significant digits, which could move an edge such as -105.123456 by tens of metres. An AQS box request's URL now matches its asset id: two boxes share one exactly when they share the other. Edges given to six decimals or fewer are sent as before, so the committed wildfire-smoke lockfile's href is unchanged; a finer box, such as one derived from `--lat`/`--lon` or a place envelope, gets a new href, and for ComCat a new asset id, on its next resolve. ([#356](https://github.com/jakeryderv/usdata/issues/356))
- The GRIB2 reader now names an MRMS `LightningProbabilityNext30minGrid_scale_1` field after its product, as it names the other MRMS products, instead of a `parameter_209_...` fallback, so its registry units (`%`) and long name are filled. ([#358](https://github.com/jakeryderv/usdata/issues/358))
- Downloads, provenance sidecars, and lockfiles are flushed to disk before and after they replace the previous file, so the lockfile-first ordering of a pull also holds across a crash or power loss. ([#359](https://github.com/jakeryderv/usdata/issues/359))
- A HRRR, RAP, GFS, or NBM window that straddles the start of the archive lists the runs the archive holds, instead of failing on an earlier run it never had.
...
- An ERDDAP coordinate axis with no units, whose CSV has an empty units row, is read with empty units instead of failing as malformed.
- HTTP downloads now write the object exactly as the source stores it. They ask for no transfer compression and write the body undecoded, so a file stored with `Content-Encoding: gzip` keeps the size and checksum its listing reports instead of being written decompressed. Services that compress on the fly still send the plain bytes they did before. A lockfile pinned by an earlier version for such an object recorded the decoded bytes; restore it with `pull --update` (or `--force`) to pin the stored bytes.
- `usdata.testing.check_utc_equivalence` now builds its naive bounds from the scenario's UTC instants, so an adapter whose scenario query is written with a non-UTC offset no longer fails the check.
