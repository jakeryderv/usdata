...
Connecticut is the one state with two sets of counties: its 2022 planning
regions, which Census geography uses, and the eight counties they replaced,
which several place-keyed sources still use. The table holds both, and such a
source refuses a region with the counties it overlaps
([places reference](../reference/places.md#connecticut)).
