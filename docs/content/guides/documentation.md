...
| First-use walkthrough | `docs/content/index.md` | Docs home (`index.md`) |
| Overview and development setup | `docs/content/project.md` | Project page (`project.md`) |
| Guides, architecture, provider access notes | Markdown under `docs/content/` | Site pages |
| Dataset status and capabilities | `src/usdata/data/registry.yaml` | `docs/content/generated/catalog/` only |
| CLI commands and options | Typer app in `src/usdata/cli/` | CLI reference during the build |
| Public Python signatures and docstrings | `src/usdata/`, selected by `docs/content/reference/api.md` | API reference during the build |
...
handwritten usage guide in `docs/content/providers/`. The site combines that guide and
...
## Build ownership and inputs

`docs/build.py`, `docs/mkdocs.yml`, and `docs/package.json` own documentation
assembly, navigation, and hosting commands. The docs npm lockfile is independent
of `web/`; no docs build or deployment command uses homepage tooling.
Python documentation dependencies remain in the repository's locked `docs` group.

The builder reads `docs/content/` and only the external files explicitly listed
in `docs/inputs.yml`: the changelog and selected runnable examples. Add a file to
that list to publish it; merely adding a file under `examples/` does not publish
it. Missing, duplicate, or out-of-scope input paths fail the build. Root README,
contribution, security, license, and release-fragment instructions are not copied
into the site. Policy links go to their canonical GitHub files; the docs-owned
project page provides detailed development setup.

Catalog facts, API/CLI references, and upcoming changes are generated from the
registry, Python source, and release fragments. These dependencies run one way:
the docs read the SDK and examples; neither depends on the website build.
...
`mkdocstrings`. Both directories are disposable. `docs/content/` maps to the existing
public `/docs/` paths, with `index.md` and `project.md` mapped to the site root.
Examples retain their `/examples/` paths. The builder adjusts relative Markdown
links for the site while source links continue to work on GitHub.
Local notebook links become rendered pages with downloads of the original notebooks.
Notebook cells are never executed during a build.
...
Edit `docs/mkdocs.yml` to change navigation outside the Datasets section. Dataset
navigation is generated from stable IDs and validated catalog metadata into
ignored `docs/.mkdocs.generated.yml`; the empty Datasets section is its insertion point. Organize by reader task, and link
