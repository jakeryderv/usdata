...
The homepage's `/datasets/` browser reads a static metadata index generated by
`scripts/render_browser.py` from the curated registry entries.
`just docs` refreshes the committed index, and `just check-docs` rejects stale
metadata or availability labels. Keep the generated index with registry and
package-version changes so the Node-only deployment needs no Python setup.
The browser links to dataset reference and usage pages in the documentation and
to example pages on the main website. It does not query live inventories or
upload dataset files.

`web/render-examples.mjs` builds `/examples/` and one page per example folder
from `examples/catalog.json`, READMEs, saved notebook outputs, and the browser's
dataset index. The index must list every example folder exactly once. Dataset
relationships come from each registry entry's `examples`; multi-source examples
should be listed under each relevant dataset. Notebook and manifest downloads
retain their exact source bytes. HTML outputs are sanitized, plots are extracted,
and code is displayed without execution. Missing relationships, saved errors, or
unsupported output formats fail the build instead of dropping content silently.

Run `npm run build --prefix web` after example edits. Generated example pages
exist only in disposable `web/dist/examples/`; do not commit a second rendered
copy. The existing Node build and automatic deployment need no Python setup.
...
directly. Examples live at `https://usdata.dev/examples/` and each example has
one page at `/examples/<example>/`, with downloads underneath it. The former
docs `/examples/` pages are no longer published. Root repository policies and
