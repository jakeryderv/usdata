...
publishing; the homepage check also exercises catalog search and filters.
...
`scripts/render_browser.py` from the curated registry and its catalog metadata.
...
relationships come from the registry's `catalog` metadata; multi-source examples
