# Providers

Generated from `src/usdata/data/registry.yaml` by `just docs`. Do not edit by hand.

Status: **available** has a tested adapter, **stub** has an adapter class that is not implemented yet, **planned** is a registry entry only.

| Provider | Available | Stub | Planned | Next up (0.5) | Datasets |
|---|---:|---:|---:|---|---|
| [NOAA](noaa.md) | 2 | 1 | 26 | `gsom`, `gsoy`, `storm-events`, `mrms`, `goes-abi`, `hurdat2`, `ibtracs`, `climate-normals`, `coops-water-levels`, `coastwatch-sst` | `ghcn-daily`, `nexrad-level2`, _coastwatch-sst_, +26 planned |
| [Census Bureau](census.md) | 0 | 0 | 1 | — | +1 planned |
| [EPA](epa.md) | 0 | 0 | 1 | — | +1 planned |
| [FEMA](fema.md) | 0 | 0 | 1 | — | +1 planned |
| [NASA](nasa.md) | 0 | 0 | 1 | — | +1 planned |
| [USDA](usda.md) | 0 | 0 | 1 | — | +1 planned |
| [USGS](usgs.md) | 0 | 0 | 3 | `water-daily` | +3 planned |
