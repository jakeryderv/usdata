...
| [Cape Henry currents](https://usdata.dev/examples/coastal-currents/) | One day of speed and direction for an explicit station and bin, gap counts, dated metadata, and fresh-cache restoration (available since v0.24.0) |
| [Hourly temperature departures](https://usdata.dev/examples/hourly-anomalies/) | One airport's routine observations against hourly normals, explicit local-standard-time matching, missing-value counts, and fresh-cache restoration (available since v0.23.0) |
...
| [GOES mesoscale evolution](https://usdata.dev/examples/goes-mesoscale/) | Fifteen minutes of one mesoscale sector, explicit timing and quality checks, a fixed-region comparison, and pinned restoration (Unreleased; available from source for v0.25.0) |
