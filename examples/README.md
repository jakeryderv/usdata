...
|---|---|
| [Severe-weather case study](https://usdata.dev/examples/severe-weather-case-study/) | One tornado report selected from the Storm Events archive, confirmed against SPC, then measured against NEXRAD, MRMS, GLM, and an HRRR analysis from one manifest with six named sources, ending in `usdata cite` (available since v0.17.0) |
