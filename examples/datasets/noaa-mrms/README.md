...
Open the [executed notebook](noaa-mrms.ipynb) to pull eleven MRMS mid-level
...
a tornado report describes. See [examples setup](https://usdata.dev/studies/#run)
to run it.
...
- `just run-notebooks --notebook mrms-rotation --write` fails with `unknown
  ...
  (`examples/datasets/noaa-mrms/noaa-mrms.ipynb`); the message neither says so nor
