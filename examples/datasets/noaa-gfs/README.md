...
[event context example](https://usdata.dev/studies/event-context/) and the
[HRRR example](https://usdata.dev/datasets/noaa/hrrr/) that reads the
...
`examples/datasets/noaa-gfs/` and use `uv run usdata` and `uv run python`.
