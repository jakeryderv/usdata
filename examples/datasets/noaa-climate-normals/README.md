...
`examples/datasets/noaa-climate-normals/` and use `uv run usdata` and `uv run python`.
...
[monthly climate notebook](https://usdata.dev/datasets/noaa/gsom/) and subtract the
normal from the observed value to get the monthly anomaly. The
[climate anomalies notebook](https://usdata.dev/studies/climate-anomalies/) does this for a
