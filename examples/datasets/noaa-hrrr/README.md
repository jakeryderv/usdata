...
Open the [executed notebook](noaa-hrrr.ipynb) to read surface CAPE and 0-3 km
...
[examples setup](https://usdata.dev/studies/#run) to run it.
...
- Report 1184052's coordinates and UTC start time are written into the notebook
  ...
  [event context example](https://usdata.dev/studies/event-context/), so that
...
- The example README this notebook replaces said the 20 UTC run is "the hour of
  ...
  [GFS example README](https://usdata.dev/datasets/noaa/gfs/) has the
...
- Refreshing one notebook needs `just run-notebooks --notebook
  examples/datasets/noaa-hrrr/noaa-hrrr.ipynb`, a repository-relative path. A slug
